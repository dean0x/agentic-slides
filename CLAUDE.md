...
A React-based slideshow presentation application for a Gen AI Hackathon education session. The app displays 62 slides covering LLM fundamentals, prompt engineering, APIs, RAG, tool use, MCP, and agents.
...
├── data/slides.js    # All 62 slides as array of objects
├── components/
│   └── Slide.jsx     # Single slide renderer with 4 layout types
...
- `type` - determines layout: `title`, `list`, `cards`, `code_split`, `mermaid_split`, `image_split`, `timeline`
...
- **code** (`type: 'code_split'`) - 40% header left, 60% dark code block right
- **mermaid_split** - same split with Mermaid diagram
